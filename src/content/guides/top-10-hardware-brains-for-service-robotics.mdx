...
```mermaid
flowchart LR
    subgraph S1["Jetson-only carrier (perception only)"]
        A1["Jetson module — AI perception"] -->|"fieldbus: integration gap"| A2["Separate motor controller"]
    end
    subgraph S2["Co-designed AI + real-time spine (one PCB)"]
        B1["Jetson module — AI perception"] --> B2["FPGA — deterministic control"]
        B2 -->|"direct motor I/O"| B3["Drives + motors"]
    end
```

*The control architecture gap: a perception-only Jetson carrier leaves an integration boundary at the fieldbus, while a co-designed AI-plus-real-time spine puts perception and deterministic control on one PCB.*

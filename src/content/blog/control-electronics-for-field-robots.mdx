...
```mermaid
flowchart LR
    subgraph D["Digital domain"]
        J["Jetson Orin NX — AI perception"]
        F["FPGA — deterministic real-time control"]
        J <--> F
    end
    ISO["Galvanic isolation barrier"]
    subgraph P["Motor + power domain"]
        FILT["Input power filtering — TVS / transient suppression"]
        DRV["Motor drive outputs"]
    end
    F --> ISO
    ISO --> DRV
    FILT --> DRV
...
*TACTUN production controller board: an FPGA real-time section and Jetson Orin NX integration on the digital side, a galvanic isolation barrier, and an isolated motor/power domain — all in a single package.*
